...
int main()
{
	...
	{ // recorrer las dos posiciones de presente y futuro en calcita
		for (i = 0; i < n; ++i)
		{
			...
			{ // asignar temperatura de calcita o varilla dependiendo de la posicion del nodo
				...
				//// Para verificar que las condiciones de frontera estan funcionando bien
				//// corro algunas simulaciones cortas donde incluyo una distribucion de temperatura
				//// diferente que permite ver la evolucion de las diferentes condiciones de frontera
				//if (i>100 && j>100)
				//{
					//Calcita_ff[k][i][j] = 80;
					//Calcita_fa[k][i][j] = 80;
	...
	ofstream salida_ff("pde_ff.txt");
	ofstream salida_fa("pde_fa.txt");
	ofstream salida_fp("pde_fp.txt");
	...
	{ 
		for (i = 0; i < n; ++i) // recorridos en el espacio
		{
			for (j = 0; j < n; ++j)
			{ 
				if (i!=0 && i!=n-1 && j!=0 && j!=n-1) // nodos interiores, sin incluir los bordes
				{
				...
				// ******************************************************************************
				if (i==0 || i==n-1 || j==0 || j==n-1) // nodos de arriba
				{
					Calcita_ff[k+1][i][j] = 10.0; // Fronteras fijas
				}

				// ******************************************************************************
				// condiciones de fronteras abiertas ********************************************
				// ******************************************************************************
				// A los nodos de los bordes les asigno el mismo valor del nodo interior mas cercano
				// Asi la derivada espacial es cero.
				if (i==0)
				{
					Calcita_fa[k+1][i][j] = Calcita_fa[k+1][i+1][j];
				}else if (i==n-1)
				{
					Calcita_fa[k+1][i][j] = Calcita_fa[k+1][i-1][j];
				}
				...
				if (j==0)
				{
					Calcita_fa[k+1][i][j] = Calcita_fa[k+1][i][j+1];
				}else if (j==n-1)
				{
					Calcita_fa[k+1][i][j] = Calcita_fa[k+1][i][j-1];
				}
				...
				if (i==0)
				{
					if (j!=0 && j!=n-1) // nodos del borde, sin incluir las esquinas
					{
						// sumar el termino del pasado
						Calcita_fp[k+1][i][j] = Calcita_fp[k][i][j]*(1.0-4.0*v*dift/(difx*difx)); 
						// sumar el termino de los vecinos en el pasado
