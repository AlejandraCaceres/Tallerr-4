...
int main()
{
	...
	{ 
			...
			{ // asignar temperatura de calcita o varilla dependiendo de la posicion del nodo
				...
				if (i==0) // nodos de arriba
				{
					...
					if (j!=0 && j!=n-1)
					{
						...
						// sumar el termino de los vecinos en el pasado
						Calcita_fp[k+1][i][j] = Calcita_fp[k+1][i][j] + (v*dift/(difx*difx)) * ( Calcita_fp[k][i+1][j] + Calcita_fp[k][n-1][j] + Calcita_fp[k][i][j+1] + Calcita_fp[k][i][j-1]);
					}
				}
			}
		}
		...
		// Cambiar referencias de presente y futuro
		for (i = 0; i < n; ++i)
		{
			for (j = 0; j < n; ++j)
			{
				Calcita_ff[k][i][j] = Calcita_ff[k+1][i][j];
				Calcita_fa[k][i][j] = Calcita_fa[k+1][i][j];
				Calcita_fp[k][i][j] = Calcita_fp[k+1][i][j];
			}
		}
